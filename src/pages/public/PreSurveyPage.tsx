...
export default function PreSurveyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!app) {
        setError('Applicant ID not found. Please check and try again.');
        return;
      }

      const { error: insertError } = await supabase.from('pre_surveys').insert({
        applicant_id: app.id,
      ...
      <div className="min-h-screen flex items-center justify-center px-6" style={{ paddingTop: 'calc(44px + 48px + 2rem)', background: 'var(--paper)' }}>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-md w-full border border-[var(--line)] p-8 text-center">
          ...
          <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '0.9rem', fontWeight: 300, color: '#3a3530' }}>
            Thank you for completing the pre-workshop survey. Your responses have been recorded.
          </p>
        </motion.div>
      </div>
    );
    ...
    <div className="min-h-screen" style={{ paddingTop: 'calc(44px + 48px + 2rem)', background: 'var(--paper)' }}>
      <div className="max-w-xl mx-auto px-6 py-12">
        ...
        <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '0.95rem', fontWeight: 300, color: '#3a3530', marginBottom: '2.5rem' }}>
          Rate your current knowledge and confidence on a scale of 1 (low) to 5 (high).
        </p>
        ...
        <form onSubmit={handleSubmit}>
                ...
                {[1, 2, 3, 4, 5].map(n => (
                  <button
                    ...
                    style={{
                      ...
                      fontSize: '1.2rem',
                      borderColor: scores[q.key] === n ? 'var(--red)' : 'var(--line)',
                      background: scores[q.key] === n ? 'var(--red)' : 'var(--paper)',
          ...
          <button
            type="submit"
            className="w-full px-8 py-3 bg-[var(--red)] text-white hover:bg-[var(--ink)] transition-colors duration-200"
